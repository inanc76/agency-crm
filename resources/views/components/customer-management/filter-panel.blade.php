        ...
        @if($showAlphabet)
            <div class="flex items-center gap-1 ml-auto flex-wrap justify-end">
                <x-mary-button 
                    ...
                    class="btn-ghost btn-xs font-medium {{ $letter === '0-9' ? 'bg-gray-100 text-blue-600' : 'text-gray-500' }} hover:bg-gray-100 px-2" 
                />
                <x-mary-button 
                    label="Tümü" 
                    wire:click="$set('letter', '')"
                    class="btn-ghost btn-xs font-medium {{ $letter === '' ? 'bg-gray-100 text-blue-600' : 'text-gray-500' }} hover:bg-gray-100 px-2" 
                />
                ...
                @foreach(range('A', 'Z') as $char)
                    <x-mary-button 
                        ...
                        class="btn-ghost btn-xs font-medium {{ $letter === $char ? 'bg-gray-100 text-blue-600' : 'text-gray-500' }} hover:bg-gray-100 min-w-[24px] !px-1" 
                    />
                @endforeach
            </div>
        @endif
