<div class="col-span-8 flex flex-col gap-6">
    ...
    <div class="theme-card p-6 shadow-sm">
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            {{-- Rapor Hedefi Switch --}}
            <div>
                <label class="block text-xs font-medium mb-1 opacity-60 text-skin-base">Rapor İlişkisi</label>
                @if($isViewMode)
                    <div class="text-sm font-medium text-skin-base">
                        @if($creation_target === 'NONE') İlişkisi Yok @elseif($creation_target === 'PROJECT') Proje @else Görev @endif
                    </div>
                @else
                    <div class="flex items-center bg-slate-100 p-1 rounded-lg w-fit">
                        ...
                        <button type="button" wire:click="$set('creation_target', 'NONE')"
                            class="px-4 py-1.5 text-xs font-bold rounded-md transition-all {{ $creation_target === 'NONE' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                            İlişkisi Yok
                        </button>
                    </div>
                @endif
        ...
        <div class="theme-card p-6 shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    @else
                        <select wire:model.live="report_type" class="select w-full" {{ !$customer_id ? 'disabled' : '' }}>
                            <option value="">Seçiniz...</option>
                            <option value="PROJECT">Yapım Projesi</option>
                            <option value="SERVICE">Destek Hizmeti</option>
                        </select>
                        @if(!$customer_id) <p class="text-red-500 text-[10px] mt-1">Lütfen önce müşteri seçiniz.</p> @endif
                ...
                @if($report_type === 'SERVICE')
                    <div>
                        ...
                        @else
                            <select wire:model.live="service_category_key" class="select w-full" {{ !$customer_id ? 'disabled' : '' }}>
                    ...
                    <div>
                        ...
                        @else
                            <select wire:model.live="project_id" class="select w-full" {{ !$customer_id ? 'disabled' : '' }}>
                                <option value="">
    ...
    @if($creation_target === 'TASK')
        <div class="theme-card p-6 shadow-sm">
            ...
            <div>
                ...
                @else
                    @if(!$customer_id)
                        <div class="w-full border border-slate-200 rounded-lg p-2 bg-slate-50 text-slate-400 text-sm cursor-not-allowed">
                            Görev seçimi için müşteri bekleniyor...
                        </div>
                        <p class="text-red-500 text-[10px] mt-1">Lütfen önce müşteri seçiniz.</p>
                    @else
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            @if(!$isViewMode)
                <div class="flex flex-col items-end gap-1">
                    <button type="button" @if(!$project_id && $creation_target === 'PROJECT')
