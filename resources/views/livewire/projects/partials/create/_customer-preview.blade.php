    ...
    @if($selectedCustomer)
        <div class="flex items-center gap-4 mb-4">
            @if($selectedCustomer["logo_url"])
                <img src="{{ str_contains($selectedCustomer["logo_url"], "/storage/") ? $selectedCustomer["logo_url"] : asset("storage" . $selectedCustomer["logo_url"]) }}"
                    alt="{{ $selectedCustomer["name"] }}" class="w-12 h-12 rounded-lg object-cover shadow-sm bg-white" />
            @else
