{{--
    ...
    @params: $showNoteModal (bool), $editingNoteId (string|null), $noteContent (string), $noteVisibleTo (array), $users (Collection)
    @events: saveNote(), closeNoteModal()
--}}

<x-mary-modal wire:model="showNoteModal"
    title="{{ $editingNoteId ? 'Notu Düzenle' : 'Yeni Not Ekle' }}" 
    class="backdrop-blur"
    box-class="!max-w-3xl">
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <p class="text-xs text-skin-muted mb-3">
                Bu notu görebilecek kullanıcıları seçin. Varsayılan olarak tüm kullanıcılar görebilir.
            </p>
            
            <x-mary-choices 
                wire:model="noteVisibleTo" 
                :options="$allUsers" 
                option-label="name" 
                option-value="id"
                searchable
            ...
            @if(count($noteVisibleTo) > 0)
                <p class="text-xs text-blue-600 mt-2 font-medium">
                    {{ count($noteVisibleTo) }} kullanıcı seçildi
                </p>
            @else
                <p class="text-xs text-amber-600 mt-2 font-medium">
                    ⚠️ En az bir kullanıcı seçmelisiniz
                </p>
            @endif
        </div>
    </div>
