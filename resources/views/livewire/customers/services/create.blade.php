        ...
        <div class="flex items-start justify-between mb-6">
            <div>
                ...
                <div class="flex items-center gap-2 mt-1">
                    @if($isViewMode)
                        <span class="text-xs font-medium px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">Hizmet</span>
        ...
        <div class="flex gap-6">
            ...
            <div class="w-4/5 space-y-6">
                ...
                <div class="theme-card p-6 shadow-sm">
                    <h2 class="text-base font-bold mb-4" style="color: var(--color-text-heading);">Varlık Seçimi</h2>
                ...
                <div class="theme-card p-6 shadow-sm border border-purple-100 bg-purple-50/50">
                    <h2 class="text-base font-bold mb-4" style="color: var(--color-text-heading);">Başlangıç Tarihi</h2>
                    <div>
                        <label class="block text-xs font-medium mb-1 opacity-60"
                            style="color: var(--color-text-base);">Başlangıç Tarihi *</label>
                        <div class="text-xs text-slate-400 mb-2">Bitiş tarihi seçilen süreye göre otomatik hesaplanacaktır.
                        </div>
                ...
                @foreach($services as $index => $service)
                    <div class="theme-card p-6 shadow-sm border border-green-100 bg-green-50/50" wire:key="service-{{ $index }}">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                @if($isViewMode)
                                    <div class="text-sm font-medium" style="color: var(--color-text-base);">{{ $service['category'] }}</div>
                                @else
                                    ...
                                    @error("services.{$index}.category") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                @if($isViewMode)
                                    <div class="text-sm font-medium" style="color: var(--color-text-base);">{{ $service['service_name'] }}
                                    </div>
                                @else
                                    <select wire:model.live="services.{{ $index }}.service_name" class="select w-full" 
                                        @if(empty($service['category'])) disabled @endif>
                                        <option value="">{{ !empty($service['category']) ? 'Hizmet seçin' : 'Kategori seçin' }}</option>
                                        @foreach($service['services_list'] ?? [] as $s)
                                            <option value="{{ $s['name'] }}">{{ $s['name'] }}</option>
                                        @endforeach
                                    </select>
                                    @error("services.{$index}.service_name") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                @if($isViewMode)
                                    <div class="text-sm font-medium" style="color: var(--color-text-base);">
                                        {{ number_format($service['service_price'], 2, ',', '.') }} {{ $service['service_currency'] }}
                                    </div>
                                @else
                                    <div class="flex items-center gap-2">
                                        <input type="number" step="0.01" wire:model="services.{{ $index }}.service_price" class="input w-full">
